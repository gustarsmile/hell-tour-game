...
**巢狀支援**：`popstate` 關掉最上層後，若下層仍開著會**補推一筆**，使返回鍵能逐層關閉。本 app 目前無巢狀（深度恆為 0 或 1），但 `artImg()` 會自動為圖片掛上大圖疊層，日後只要有人在選單或善書冊內放一張圖，巢狀即無聲出現；少了這步，第二次按返回鍵會直接退出遊戲。

**已知競態（不處理）**：`back()` 已呼叫但 `popstate` 尚未抵達的那幾毫秒內，若使用者恰好手動按下返回鍵，歷程位置會少一格。窗口極短，且此競態在被廢除的舊設計中同樣存在且更嚴重，非本設計引入。
...
理由：這些 `<img>` 的 `alt` 為空字串（裝飾性語意）。加上 `tabindex`／`role="button"` 而不同時撰寫可辨識名稱，等於製造一批無名稱的可聚焦控制項，對螢幕閱讀器使用者是淨負面（WCAG 4.1.2）。真正的前置條件是先為 41 張圖撰寫 alt 文字，而那已被本設計的 I 節明列為範圍外。

代價：**開啟大圖需要滑鼠或觸控**，鍵盤使用者目前無法開啟大圖疊層，此為已知且刻意的限制，並非「開啟後即無障礙」的緩解可以抵銷。相對地，漢堡選單與善書冊本來就能以鍵盤觸發開啟，這次新增的 Esc 對它們是實質改善 — ✕ 鈕可 focus、Esc 可關。
...
5. 開關一輪後歷程成對：`pushState` 與 `back` 各呼叫一次（對帳在 microtask，需 await）
6. 堆疊為空時 Esc 與 `popstate` 皆為 no-op
7. `pushState` 拋錯（模擬 `file://`）時仍可正常關閉，且不呼叫 `history.back()`
8. 多次開關不重複綁定 document 監聽器
9. **關鍵回歸**：關閉一層後同步開啟另一層，**完全不得呼叫 `back()`**，且全程只 `pushState` 一次（`menuAction` 的 `close(); fn();` 情境 — 這正是造成 about:blank 缺陷的路徑）
10. `back()` 回音在途期間開新層，回音抵達時補推歷程
11. 巢狀時返回鍵逐層關閉：關掉頂層後為下層補推歷程
...
12. 「翻閱善書冊」端到端：經選單項目開啟後善書冊**仍在畫面上**（驗證歷程對帳在真實呼叫路徑上生效）
