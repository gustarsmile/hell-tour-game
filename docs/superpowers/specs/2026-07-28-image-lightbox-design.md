...
**巢狀支援**： 關掉最上層後，若下層仍開著會**補推一筆**，使返回鍵能逐層關閉。本 app 目前無巢狀（深度恆為 0 或 1），但  會自動為圖片掛上大圖疊層，日後只要有人在選單或善書冊內放一張圖，巢狀即無聲出現；少了這步，第二次按返回鍵會直接退出遊戲。

**已知競態（不處理）**： 已呼叫但 popstate 尚未抵達的那幾毫秒內，若使用者恰好手動按下返回鍵，歷程位置會少一格。窗口極短，且此競態在被廢除的舊設計中同樣存在且更嚴重，非本設計引入。
